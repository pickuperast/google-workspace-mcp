...
- Every push to the `main` branch must include an application version bump in `package.json` and `package-lock.json`.
- When making dramatic feature changes, update `README.md` in the same change so setup, usage, tool lists, and limitations stay current.
- After dramatic feature changes, review this `AGENTS.md` file and self-update it when repository instructions, source-file maps, tool categories, limitations, or workflows have changed.
